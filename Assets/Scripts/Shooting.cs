		...
		if (Input.GetButtonUp ("Fire1") && ! GameManager.Instance.IsMenuShowing) {
			if (waitTilFire <= 0) {
				if (bullet) {
					...
					shot++;
					Physics.IgnoreCollision (transform.root.collider, obj.transform.collider);
					foreach (Collider c in transform.root.gameObject.GetComponentsInChildren<Collider>()) {
						Physics.IgnoreCollision (c, obj.transform.collider);
					}
					obj.rigidbody.velocity = transform.root.rigidbody.velocity +
